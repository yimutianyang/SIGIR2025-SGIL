...
class Dataset(object):
    ...
    def _batch_sampling_softmax(self):
        batch_num = int(len(self.training_user) / self.batch_size) + 1
        indexs = np.arange(len(self.training_user))
        training_user, training_item = np.array(self.training_user), np.array(self.training_item)
        np.random.shuffle(indexs)
        for k in range(batch_num):
            index_start = k * self.batch_size
            index_end = min((k + 1) * self.batch_size, len(indexs))
            if index_end == len(indexs):
                index_start = len(indexs) - self.batch_size
            yield training_user[indexs[index_start:index_end]], training_item[indexs[index_start:index_end]]
